...
#define scap_debug_log(HANDLE, ...) scap_log_impl(HANDLE->m_log_fn, KHULNASOFT_LOG_SEV_DEBUG, __VA_ARGS__)

/**
 * If debug_log_fn has been established in the handle, call that function
 * to log a debug message.
 */
static inline void scap_log_impl(khulnasoft_log_fn log_fn, enum khulnasoft_log_severity sev, const char* fmt, ...)
{
	if(log_fn != NULL)
	{
		char buf[256];
