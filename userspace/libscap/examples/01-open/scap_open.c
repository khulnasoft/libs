...
static enum khulnasoft_log_severity severity_level = KHULNASOFT_LOG_SEV_WARNING;
...
void parse_CLI_options(int argc, char** argv)
{
	for(int i = 0; i < argc; i++)
	{
		...
		if(!strcmp(argv[i], VERBOSE_OPTION))
		{
			...
			severity_level = (enum khulnasoft_log_severity)level;
		}
...
void scap_open_log_fn(const char* component, const char* msg, const enum khulnasoft_log_severity sev)
{
