...
{
	enum khulnasoft_log_severity sev;
	switch(level)
	{
	case LIBBPF_WARN:
		sev = KHULNASOFT_LOG_SEV_WARNING;
		break;
	case LIBBPF_INFO:
		sev = KHULNASOFT_LOG_SEV_INFO;
		break;
	case LIBBPF_DEBUG:
		sev = KHULNASOFT_LOG_SEV_DEBUG;
		break;
	default:
		return -EINVAL;
	}
...
int pman_init_state(khulnasoft_log_fn log_fn, unsigned long buf_bytes_dim, uint16_t cpus_for_each_buffer,
		    bool allocate_online_only)
{
	 ...
	 * Khulnasoft issue: https://github.com/khulnasoft/khulnasoft/issues/2626
