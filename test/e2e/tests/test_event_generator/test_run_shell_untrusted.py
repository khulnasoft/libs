    ...
    expected_events = [
        {
            ...
            "evt.args": SinspField.regex_field(r'^res=0 exe=\/tmp\/khulnasoft-event-generator\d+\/httpd args=--loglevel.info.run.\^helper.RunShell\$. tid=\d+\(httpd\) pid=\d+\(httpd\) ptid=\d+\(event-generator\) .* tty=0 pgid=\d+\(systemd\) loginuid=-1\(\<NONE\>\) flags=1\(EXE_WRITABLE\) cap_inheritable=0'),
            ...
            "proc.exe": SinspField.regex_field(r'^\/tmp\/khulnasoft-event-generator\d+\/httpd$'),
