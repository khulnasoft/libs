...
# Building the modern-khulnasoft-builder can take a very very long time (> 40min).
...
- Modify the kernel grid by changing the URLs in the [kernels.jsonl](kernels.jsonl) file. This allows you to customize the entire test suite. When it comes to Amazon Linux distros, you can refer to [this link](https://github.com/khulnasoft/kernel-crawler/issues/145) to learn how to retrieve the `kernel` and `kernel-devel` RPMs. When downloading new RPMs, it is necessary to delete/remove the existing folders (`libs/test/vm/build/kernels/`, `libs/test/vm/build/headers/`, `libs/test/vm/build/headers_extracted/`) first.
...
- Note: This test framework currently does not support Apple silicon, but Khulnasoft has published a [blog post](https://khulnasoft.com/blog/khulnasoft-apple-silicon/) specifically addressing the usage of Khulnasoft in a Linux VM on macOS with Apple Silicon made possible by Khulnasoft's aarch64 support.
...
- Not intended for [driverkit](https://github.com/khulnasoft/driverkit) integration.
...
To build the userspace binary, the officially supported khulnasoft-builder container is pulled. Otherwise, custom containers are built to cater to the specific requirements of the test suites.
...
Historical results tables are preserved [here](https://github.com/khulnasoft/libs/issues/982).
