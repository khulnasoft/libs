...
static khulnasoft_log_severity severity_level = KHULNASOFT_LOG_SEV_WARNING;
...
void test_open_log_fn(const char* component, const char* msg, khulnasoft_log_severity sev)
{
		...
		case 'v':
			{
				...
				if(level < KHULNASOFT_LOG_SEV_FATAL || level > KHULNASOFT_LOG_SEV_TRACE)
				{
					std::cerr << "Invalid logging level. Valid range is '" << std::to_string(KHULNASOFT_LOG_SEV_FATAL) <<"' <= lev <= '" << std::to_string(KHULNASOFT_LOG_SEV_TRACE) << "'" << std::endl;
					return EXIT_FAILURE;
				}
				severity_level = (khulnasoft_log_severity)level;
			}
			break;
