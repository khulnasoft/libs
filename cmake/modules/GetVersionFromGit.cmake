...
if(_khulnasoft_get_version_from_git)
    return()
endif()

set(_khulnasoft_get_version_from_git TRUE)

if(NOT KHULNASOFT_RELEASE_BRANCH_REGEX)
    set(KHULNASOFT_RELEASE_BRANCH_REGEX "^release/")
endif()

function(_khulnasoft_execute_git _out)
    if(NOT GIT_FOUND)
...
function(_khulnasoft_extract_version version_str x y z found)
...
# get_version_from_git() provides a basic implementation of the Khulnasoft versioning convention.
#
...
function(get_version_from_git _var match_suffix exclude_suffix)
    ...
    if(exclude_suffix)
        _khulnasoft_execute_git(tag describe --tags --exact-match --match "*${match_suffix}" --exclude "*${exclude_suffix}" HEAD)
    else()
        _khulnasoft_execute_git(tag describe --tags --exact-match --match "*${match_suffix}" HEAD)
    endif()
    ...
    _khulnasoft_execute_git(current_hash rev-parse HEAD)
    _khulnasoft_execute_git(current_hash_short rev-parse --short=7 HEAD)
    _khulnasoft_execute_git(current_branch rev-parse --abbrev-ref HEAD)

    set(dev_version "0.0.0")

    # dev version / in a `release/M.m.x` branch
    if(current_branch MATCHES "${KHULNASOFT_RELEASE_BRANCH_REGEX}")
        ...
        _khulnasoft_execute_git(tag describe --tags ${_options} --abbrev=0 "${current_branch}")

        if(tag)
            _khulnasoft_extract_version("${tag}" x y z match_found)
    ...
    if(dev_version MATCHES "0.0.0")
        ...
        _khulnasoft_execute_git(tag for-each-ref --count=1 --sort=-version:refname --format "%(refname:short)" refs/tags/[0-9]*.[0-9]*.[0-9]${match_suffix})
    ...
    if(NOT dev_version MATCHES "0.0.0")
        _khulnasoft_execute_git(tag_commit rev-list -n 1 "${tag}")
        _khulnasoft_execute_git(tag_common_ancestor merge-base "${current_hash}" "${tag_commit}")
        _khulnasoft_execute_git(commits_count rev-list --count "${tag_common_ancestor}..${current_hash}")
